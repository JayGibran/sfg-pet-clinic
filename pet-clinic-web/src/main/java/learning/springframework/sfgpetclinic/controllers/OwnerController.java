...
public class OwnerController {
    ...
    @GetMapping("/new")
    public String showNewOrUpdateOwne(Model model) {
    ...
    @PostMapping("/new")
    public String addNewOrUpdate (@Valid Owner owner, BindingResult result) {
        if(result.hasErrors()){
            return VIEW_OWNER_CREATE_OR_UPDATE_FORM;
        }else{
    ...
    public String processUpdateOwnerForm(@Valid Owner owner, BindingResult result, @PathVariable Long ownerId){
        if(result.hasErrors()){
            ...
            owner.setId(ownerId);
            Owner ownerUpdate = this.ownerService.save(owner);
            return "redirect:/owners/" + ownerUpdate.getId();
        }
    }
